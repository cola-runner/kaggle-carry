...
def _add_card_counts(
    ...
    for card_id, count in counts.items():
        key = f"count:{prefix}:{card_id}"
        features[key] = float(min(features.get(key, 0.0) + count, 8))
...
def _add_board_features(
    ...
    bench = _objects(player.get("bench"))
    board = active + bench
    ...
    for card in board:
        _add_card_counts(
            features,
            f"{prefix}_board_energy_card",
            card.get("energyCards"),
        )
        _add_card_counts(
            features,
            f"{prefix}_board_tool_card",
            card.get("tools"),
        )
        _add_card_counts(
            features,
            f"{prefix}_board_evolution_card",
            card.get("preEvolution"),
        )

    for zone, cards in (("active", active), ("bench", bench)):
        for index, card in enumerate(cards):
            slot = f"{zone}_{index}"
            card_id = _integer(card.get("id"))
            hp = _number(card.get("hp"))
            maximum_hp = _number(card.get("maxHp"))
            energy_cards = _objects(card.get("energyCards"))
            tools = _objects(card.get("tools"))
            evolution = _objects(card.get("preEvolution"))
            energy_count = len(energy_cards)
            tool_count = len(tools)
            features[f"n:{prefix}_{slot}_hp"] = hp
            features[f"n:{prefix}_{slot}_max_hp"] = maximum_hp
            features[f"n:{prefix}_{slot}_damage"] = max(0.0, maximum_hp - hp)
            features[f"n:{prefix}_{slot}_energy"] = float(energy_count)
            features[f"n:{prefix}_{slot}_tools"] = float(tool_count)
            features[f"n:{prefix}_{slot}_evolution_depth"] = float(
                len(evolution)
            )
            features[f"n:{prefix}_{slot}_appear_this_turn"] = float(
                bool(card.get("appearThisTurn"))
            )
            if card_id > 0:
                features[f"cat:{prefix}_{slot}_card={card_id}"] = 1.0
                features[
                    f"cat:{prefix}_{slot}_card_state="
                    f"{card_id}:{energy_count}:{tool_count}"
                ] = 1.0
            for attachment_prefix, attachments in (
                ("energy", energy_cards),
                ("tool", tools),
                ("evolution", evolution),
            ):
                for attachment in attachments:
                    ...
                    if attachment_id <= 0:
                        continue
                    features[
                        f"cat:{prefix}_{slot}_{attachment_prefix}="
                        f"{card_id}:{attachment_id}"
                    ] = 1.0
...
def visible_state_features(
    ...
    features: dict[str, float] = {
        ...
        "n:first_player": _number(current.get("firstPlayer"), -1.0),
        "n:first_player_is_me": float(
            _integer(current.get("firstPlayer"), -1) == seat
        ),
        ...
        "n:me_bench_max": _number(me.get("benchMax")),
        ...
        "n:rival_bench_max": _number(rival.get("benchMax")),
        f"cat:seat={seat}": 1.0,
    }
    ...
    for offset, log in enumerate(reversed(logs[-8:])):
        features[
            "cat:recent_log="
            f"{offset}:{_integer(log.get('type'), -1)}:"
            f"{_integer(log.get('cardId'))}:"
            f"{_integer(log.get('playerIndex'), -1)}"
        ] = 1.0
    ...
    if isinstance(select, Mapping):
        ...
        effect_id = (
            _integer(effect.get("id"))
            if isinstance(effect, Mapping)
            else 0
        )
        context_card = select.get("contextCard")
        context_card_id = (
            _integer(context_card.get("id"))
            if isinstance(context_card, Mapping)
            else 0
        )
        if effect_id > 0:
            features[f"cat:select_effect_card={effect_id}"] = 1.0
        if context_card_id > 0:
            features[f"cat:select_context_card={context_card_id}"] = 1.0
...
def visible_action_features(
    ...
    *,
    ...
    def add_card_metrics(prefix: str, card: object) -> None:
        ...
        features[f"n:action_{prefix}_appear_this_turn"] = float(
            bool(card.get("appearThisTurn"))
        )
    ...
    rival = (
        players[rival_seat]
        if rival_seat < len(players)
        and isinstance(players[rival_seat], Mapping)
        else {}
    )
    me_active = _objects(me.get("active"))
    rival_active = _objects(rival.get("active"))
    me_active_id = _integer(me_active[0].get("id")) if me_active else 0
    rival_active_id = (
        _integer(rival_active[0].get("id")) if rival_active else 0
    )
    action_categories[
        "cat:action_matchup="
        f"{signature[0]}:{signature[1]}:{signature[3]}:"
        f"{me_active_id}:{rival_active_id}"
    ] = 1.0
    ...
    if isinstance(select, Mapping):
        ...
        action_categories[
            f"cat:context={context}|source={signature[1]}"
        ] = 1.0
        if effect_id > 0:
            action_categories[
                "cat:context="
                f"{context}|effect={effect_id}|source={signature[1]}"
            ] = 1.0
