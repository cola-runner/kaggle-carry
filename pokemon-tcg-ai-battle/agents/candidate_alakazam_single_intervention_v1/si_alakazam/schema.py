...
class SnapshotManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.minimum_teacher_teams < 1:
            raise ValueError("minimum_teacher_teams must be positive")
        if (
            len({teacher.team_id for teacher in self.teachers})
            < self.minimum_teacher_teams
        ):
            raise ValueError(
                "teachers must contain at least "
                f"{self.minimum_teacher_teams} distinct teams"
            )
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "minimum_teacher_teams": self.minimum_teacher_teams,
        }
